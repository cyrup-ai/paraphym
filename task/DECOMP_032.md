# DECOMP_032: Decompose `nvembed.rs`

**File:** `packages/candle/src/capability/text_embedding/nvembed.rs`  
**Current Size:** 715 lines  
**Module Area:** capability / text_embedding

## OBJECTIVE

Decompose the monolithic `nvembed.rs` (715 lines) into smaller, focused, maintainable modules while preserving all existing functionality.
...
Read and analyze `packages/candle/src/capability/text_embedding/nvembed.rs` to identify:
...
- [ ] `nvembed.rs` is reduced to < 300 lines (ideally much less)
...
- File size (715 lines) indicates high complexity or multiple responsibilities
