# DECOMP_036: Decompose `jina_bert.rs`

**File:** `packages/candle/src/capability/text_embedding/jina_bert.rs`  
**Current Size:** 688 lines  
**Module Area:** capability / text_embedding

## OBJECTIVE

Decompose the monolithic `jina_bert.rs` (688 lines) into smaller, focused, maintainable modules while preserving all existing functionality.
...
Read and analyze `packages/candle/src/capability/text_embedding/jina_bert.rs` to identify:
...
- [ ] `jina_bert.rs` is reduced to < 300 lines (ideally much less)
...
- File size (688 lines) indicates high complexity or multiple responsibilities
