...
Decompose the monolithic `parsing.rs` (894 lines) into smaller, focused, maintainable modules while preserving all existing functionality.
...
Read and analyze `packages/candle/src/domain/chat/commands/parsing.rs` to identify:
...
- [ ] `parsing.rs` is reduced to < 300 lines (ideally much less)
...
### File Location
`packages/candle/src/domain/chat/commands/parsing.rs`

### Key Considerations
- This file is part of the **domain / chat / commands** module area
- File size (894 lines) indicates high complexity or multiple responsibilities
