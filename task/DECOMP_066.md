# DECOMP_066: Decompose `info.rs`

**File:** `packages/candle/src/domain/model/info.rs`  
**Current Size:** 518 lines  
**Module Area:** domain / model

## OBJECTIVE

Decompose the monolithic `info.rs` (518 lines) into smaller, focused, maintainable modules while preserving all existing functionality.
...
Read and analyze `packages/candle/src/domain/model/info.rs` to identify:
...
- [ ] `info.rs` is reduced to < 300 lines (ideally much less)
...
### File Location
`packages/candle/src/domain/model/info.rs`

### Key Considerations
- This file is part of the **domain / model** module area
- File size (518 lines) indicates high complexity or multiple responsibilities
