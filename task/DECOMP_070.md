# DECOMP_070: Decompose `json_util.rs`

**File:** `packages/candle/src/domain/util/json_util.rs`  
**Current Size:** 503 lines  
**Module Area:** domain / util

## OBJECTIVE

Decompose the monolithic `json_util.rs` (503 lines) into smaller, focused, maintainable modules while preserving all existing functionality.
...
Read and analyze `packages/candle/src/domain/util/json_util.rs` to identify:
...
- [ ] `json_util.rs` is reduced to < 300 lines (ideally much less)
...
### Key Considerations
- This file is part of the **domain / util** module area
- File size (503 lines) indicates high complexity or multiple responsibilities
