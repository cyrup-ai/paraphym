# DECOMP_015: Decompose `parser.rs`

**File:** `packages/candle/src/domain/chat/templates/parser.rs`  
**Current Size:** 942 lines  
**Module Area:** domain / chat / templates

## OBJECTIVE

Decompose the monolithic `parser.rs` (942 lines) into smaller, focused, maintainable modules while preserving all existing functionality.
...
Read and analyze `packages/candle/src/domain/chat/templates/parser.rs` to identify:
...
- [ ] `parser.rs` is reduced to < 300 lines (ideally much less)
...
- File size (942 lines) indicates high complexity or multiple responsibilities
