# DECOMP_058: Decompose `metadata.rs`

**File:** `packages/candle/src/domain/chat/commands/types/metadata.rs`  
**Current Size:** 559 lines  
**Module Area:** domain / chat / commands / types

## OBJECTIVE

Decompose the monolithic `metadata.rs` (559 lines) into smaller, focused, maintainable modules while preserving all existing functionality.
...
Read and analyze `packages/candle/src/domain/chat/commands/types/metadata.rs` to identify:
...
- [ ] `metadata.rs` is reduced to < 300 lines (ideally much less)
...
### File Location
`packages/candle/src/domain/chat/commands/types/metadata.rs`

### Key Considerations
- This file is part of the **domain / chat / commands / types** module area
- File size (559 lines) indicates high complexity or multiple responsibilities
