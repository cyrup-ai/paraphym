...
Decompose the monolithic `image.rs` (746 lines) into smaller, focused, maintainable modules while preserving all existing functionality.
...
Read and analyze `packages/candle/src/builders/image.rs` to identify:
...
- [ ] `image.rs` is reduced to < 300 lines (ideally much less)
...
- File size (746 lines) indicates high complexity or multiple responsibilities
