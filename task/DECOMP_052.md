...
Decompose the monolithic `chat.rs` (593 lines) into smaller, focused, maintainable modules while preserving all existing functionality.
...
Read and analyze `packages/candle/src/domain/agent/chat.rs` to identify:
...
- [ ] `chat.rs` is reduced to < 300 lines (ideally much less)
...
- File size (593 lines) indicates high complexity or multiple responsibilities
