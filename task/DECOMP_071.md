# DECOMP_071: Decompose `worker.rs`

**File:** `packages/candle/src/capability/text_to_image/stable_diffusion_35_turbo/worker.rs`  
**Current Size:** 501 lines  
**Module Area:** capability / text_to_image / stable_diffusion_35_turbo

## OBJECTIVE

Decompose the monolithic `worker.rs` (501 lines) into smaller, focused, maintainable modules while preserving all existing functionality.
...
Read and analyze `packages/candle/src/capability/text_to_image/stable_diffusion_35_turbo/worker.rs` to identify:
...
- [ ] `worker.rs` is reduced to < 300 lines (ideally much less)
...
### File Location
`packages/candle/src/capability/text_to_image/stable_diffusion_35_turbo/worker.rs`

### Key Considerations
- This file is part of the **capability / text_to_image / stable_diffusion_35_turbo** module area
- File size (501 lines) indicates high complexity or multiple responsibilities
