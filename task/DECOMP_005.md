# DECOMP_005: Decompose `config.rs`

**File:** `packages/candle/src/domain/chat/config.rs`  
**Current Size:** 1,469 lines  
**Module Area:** domain / chat

## OBJECTIVE

Decompose the monolithic `config.rs` (1,469 lines) into smaller, focused, maintainable modules while preserving all existing functionality.
...
Read and analyze `packages/candle/src/domain/chat/config.rs` to identify:
...
- [ ] `config.rs` is reduced to < 300 lines (ideally much less)
...
- File size (1,469 lines) indicates high complexity or multiple responsibilities
