# DECOMP_006: Decompose `coordinator.rs`
...
Decompose the monolithic `coordinator.rs` (1,330 lines) into smaller, focused, maintainable modules while preserving all existing functionality.
...
Read and analyze `packages/candle/src/memory/core/manager/coordinator.rs` to identify:
...
- [ ] `coordinator.rs` is reduced to < 300 lines (ideally much less)
...
### Key Considerations
- This file is part of the **memory / core / manager** module area
- File size (1,330 lines) indicates high complexity or multiple responsibilities
