...
**Current Size:** 1,116 lines  
**Module Area:** domain / context

## OBJECTIVE

Decompose the monolithic `chunk.rs` (1,116 lines) into smaller, focused, maintainable modules while preserving all existing functionality.
...
Read and analyze `packages/candle/src/domain/context/chunk.rs` to identify:
...
- [ ] `chunk.rs` is reduced to < 300 lines (ideally much less)
...
### File Location
`packages/candle/src/domain/context/chunk.rs`

### Key Considerations
- This file is part of the **domain / context** module area
- File size (1,116 lines) indicates high complexity or multiple responsibilities
