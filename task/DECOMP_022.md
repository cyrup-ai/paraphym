# DECOMP_022: Decompose `stella.rs`
...
Decompose the monolithic `stella.rs` (883 lines) into smaller, focused, maintainable modules while preserving all existing functionality.
...
Read and analyze `packages/candle/src/capability/text_embedding/stella.rs` to identify:
...
- [ ] `stella.rs` is reduced to < 300 lines (ideally much less)
...
- File size (883 lines) indicates high complexity or multiple responsibilities
