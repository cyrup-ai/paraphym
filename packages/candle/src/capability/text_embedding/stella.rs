...
impl StellaEmbeddingModel {
    ...
    fn embed_dim(&self, dimension: u32) -> std::result::Result<EmbedDim, Box<dyn std::error::Error + Send + Sync>> {
        match dimension {
...
impl crate::capability::traits::TextEmbeddingCapable for StellaEmbeddingModel {
        ...
        -> std::result::Result<Vec<f32>, Box<dyn std::error::Error + Send + Sync>> 
    {
        ...
        let dimension = self.info().embedding_dimension
        ...
        -> std::result::Result<Vec<Vec<f32>>, Box<dyn std::error::Error + Send + Sync>> 
    {
        ...
        let max_length = self.info().max_input_tokens
            .ok_or("max_input_tokens missing")?.get() as usize;
        let dimension = self.info().embedding_dimension
        ...
        let tokens = tokenizer.encode_batch(formatted_refs, true)
            .map_err(|e| format!("Tokenization failed: {}", e))?;
