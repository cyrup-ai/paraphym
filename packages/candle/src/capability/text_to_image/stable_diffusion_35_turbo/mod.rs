...
struct TripleClipEncoderConfig<'a> {
    clip_g_file: &'a PathBuf,
    clip_l_file: &'a PathBuf,
    t5xxl_file: &'a PathBuf,
    clip_l_tokenizer_path: &'a PathBuf,
    clip_g_tokenizer_path: &'a PathBuf,
    t5_config_path: &'a PathBuf,
    t5_tokenizer_path: &'a PathBuf,
    device: &'a Device,
}
...
impl ImageGenerationModel for StableDiffusion35Turbo {
    fn generate(
        ...
        AsyncStream::with_channel(move |sender| {
            ...
            let mut triple_clip = match TripleClipEncoder::load(TripleClipEncoderConfig {
...
impl TripleClipEncoder {
    ...
    fn encode_prompt(&mut self, prompt: &str, device: &Device) -> Result<(Tensor, Tensor), String> {
