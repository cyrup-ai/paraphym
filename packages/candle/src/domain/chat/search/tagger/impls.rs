...
impl CandleConversationTagger {
    ...
    pub fn add_tag(&self, message_id: &str, tag_id: &str) {
        ...
        let mut msg_tags = self.message_tags
            .get(message_id)
            .map(|e| e.value().clone())
            .unwrap_or_default();
        ...
        let mut tag_msgs = self.tag_messages
            .get(tag_id)
            .map(|e| e.value().clone())
            .unwrap_or_default();
    ...
    fn initialize_default_rules(&mut self) {
        ...
        if let Ok(pattern) = Regex::new(r"(?i)(error|exception|failed|failure|panic|crash|fatal|abort)") {
        ...
        if let Ok(pattern) = Regex::new(r"(```[\s\S]*?```|`[^`]+`|fn\s+\w+|function\s+\w+|def\s+\w+|class\s+\w+|struct\s+\w+|impl\s+\w+)") {
        ...
        // Command detection
        if let Ok(pattern) = Regex::new(r"(?:^\$|^>|^#|\bcargo\s+\w+|\bnpm\s+\w+|\bgit\s+\w+|\bpython\s+|\.\/\w+)") {
    ...
    pub fn tag_statistics(&self) -> CandleTaggingStatistics {
        ...
        let avg_tags_per_message = if tagged_messages > 0 {
            let total_tag_assignments: usize = self.message_tags
                .iter()
                .map(|e| e.value().len())
                .sum();
