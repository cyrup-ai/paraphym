    ...
    pub fn validate_command(&self, command: &ImmutableChatCommand) -> Result<(), ValidationError> {
        match command {
            ...
            ImmutableChatCommand::Config { key, value, .. } => {
                self.config.validate_config_command(key.as_deref(), value.as_deref())
            }
            ...
            ImmutableChatCommand::Template {
                ...
                variables,
                ..
            } => self.config.validate_template_command(name.as_deref(), content.as_deref(), variables),
            ImmutableChatCommand::Macro { name, commands, .. } => {
                self.config.validate_macro_command(name.as_deref(), commands)
            }
            ImmutableChatCommand::Branch { name, source, .. } => {
                self.config.validate_branch_command(name.as_deref(), source.as_deref())
            }
            ...
            ImmutableChatCommand::Theme {
                name, properties, ..
            } => self.config.validate_theme_command(name.as_deref(), properties),
            ...
            ImmutableChatCommand::Save { name, location, .. } => {
                self.config.validate_save_command(name.as_deref(), location.as_deref())
            }
            ImmutableChatCommand::Load { name, location, .. } => {
                self.config.validate_load_command(name, location.as_deref())
            }
            ImmutableChatCommand::Import { source, .. } => self.config.validate_import_command(source),
            ImmutableChatCommand::Settings { key, value, .. } => {
                self.config.validate_settings_command(key.as_deref(), value.as_deref())
            }
