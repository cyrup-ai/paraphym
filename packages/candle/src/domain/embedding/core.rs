...
pub trait EmbeddingModel: Send + Sync + Clone + std::fmt::Debug {
    ...
    /// Performance: Zero allocation, direct streaming without futures using pure streams architecture
    fn on_embedding<F>(&self, text: &str, handler: F) -> crate::AsyncStream<EmbeddingChunk>
    where
