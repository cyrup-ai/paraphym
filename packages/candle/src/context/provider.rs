...
impl CandleContext<CandleFiles> {
    ...
    /// Load documents asynchronously with streaming - streams individual documents
    #[inline]
    pub fn load(self) -> AsyncStream<CandleDocument> {
        AsyncStream::with_channel(move |sender| {
            spawn_task(move || {
                match self.source {
                    CandleContextSourceType::Files(files_context) => {
                        // Expand glob pattern and load files - stream each document individually
                            ...
                            Ok(paths) => {
                                for entry in paths.flatten() {
                                    if let Ok(content) = std::fs::read_to_string(&entry) {
                                        ...
                                        // Stream individual documents as they're loaded
            ...
            spawn_task(move || {
                match self.source {
                    CandleContextSourceType::Directory(directory_context) => {
                        // Traverse directory and stream individual files
                        fn traverse_dir(
                            path: &str,
                            recursive: bool,
                            extensions: &[String],
                            ...
                            current_depth: usize,
                            sender: &AsyncStreamSender<CandleDocument>,
                        ) -> Result<(), std::io::Error> {
                                ...
                                if path.is_file() {
                                    ...
                                    if should_include {
                                        if let Ok(content) = std::fs::read_to_string(&path) {
                                            ...
                                            // Stream individual documents as they're found
                                            if sender.send(document).is_err() {
                        ...
                        if let Err(_e) = traverse_dir(
                            ...
                            // Error handled by ystream::handle_error! below
                        }
                                        e
                                    )),
