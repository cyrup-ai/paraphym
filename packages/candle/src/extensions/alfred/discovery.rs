...
async fn parse_workflow_metadata(
    ...
    let dict = match plist {
        ...
        _ => return Err(ExtensionError::MetadataParseError("Invalid plist structure".to_string())),
    };
    ...
    let bundle_id = dict.get("bundleid")
        .and_then(|v| v.as_string())
        .unwrap_or("unknown")
        .to_string();
    ...
    if let Some(objects) = dict.get("objects").and_then(|v| v.as_array()) {
        for obj in objects {
            if let Value::Dictionary(obj_dict) = obj {
                let obj_type = obj_dict.get("type")
                    .and_then(|v| v.as_string())
                    .unwrap_or("");
...
fn parse_script_filter(dict: &plist::Dictionary) -> Result<AlfredScriptFilter> {
    ...
    let script = dict.get("script")
        .and_then(|v| v.as_string())
        .unwrap_or("")
        .to_string();
    
    let script_lang = dict.get("scriptargtype")
