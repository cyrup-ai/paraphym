...
pub struct Application {
    ...
    runtime: Arc<tokio::runtime::Runtime>, // Tokio runtime for async operations
            ...
            match TerminalPane::new(event_loop, proxy, &self.config) {
                Ok(mut pane) => {
                    ...
                    let test_msg = concat!(
                        "\r\n",
                        "\x1b[1;33m╔════════════════════════════════════════╗\x1b[0m\r\n",
                        "\x1b[1;33m║\x1b[0m  \x1b[1;32mSIXEL6VT TERMINAL ACTIVE\x1b[0m         \x1b[1;33m║\x1b[0m\r\n",
                        "\x1b[1;33m║\x1b[0m  Basic text rendering works!        \x1b[1;33m║\x1b[0m\r\n",
                        "\x1b[1;33m║\x1b[0m  Loading browser screenshot...       \x1b[1;33m║\x1b[0m\r\n",
                        "\x1b[1;33m╚════════════════════════════════════════╝\x1b[0m\r\n",
                        "\r\n"
                    );
        ...
        match event.payload {
            RioEventType::Rio(RioEvent::PtyWrite(text)) => {
                if let Some(terminal_pane) = self.terminal_pane.as_mut() {
                    // Log for debugging
                    if text.len() > 1000 {
                        info!("PtyWrite event received: {} bytes (LARGE - sixel data)", text.len());
                    } else {
