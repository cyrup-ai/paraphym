...
/// Validation check results grouped together to reduce parameter count
#[derive(Debug, Clone)]
struct ValidationChecks {
    time: CheckResult,
    ocsp: CheckResult,
    crl: CheckResult,
    ca: Option<CheckResult>,
    domain: Option<CheckResult>,
}
...
impl CertificateValidatorWithInput {
    ...
    pub async fn validate(self) -> CertificateValidationResponse {
        ...
        // Parse certificate
        let parsed_cert = match Self::parse_certificate_content(
            &cert_content,
            &mut validation_breakdown,
            start_time,
        ) {
            Ok(cert) => cert,
            Err(response) => return *response,
        ...
        let time_check =
            Self::validate_certificate_time(&parsed_cert, &mut issues, &mut validation_breakdown);
        ...
        let Some(tls_manager) = self.create_tls_manager(&mut issues).await else {
        ...
        let (ocsp_check, crl_check) = self
            .perform_revocation_checks(
                &cert_content,
                &tls_manager,
                &mut issues,
                &mut validation_breakdown,
            ...
            .await;
        let ca_check = self
            .validate_certificate_chain(&cert_content, &mut issues, &mut validation_breakdown)
            .await;
    ...
    fn load_certificate_content(
        &self,
        start_time: Instant,
        validation_breakdown: &HashMap<String, std::time::Duration>,
    ) -> Result<String, super::super::responses::BoxedCertificateValidationResponse> {
        match &self.input_source {
            InputSource::File(path) => match std::fs::read_to_string(path) {
                Ok(content) => Ok(content),
                Err(e) => Err(Box::new(Self::create_error_response(
                    start_time,
    ...
    fn parse_certificate_content(
        cert_content: &str,
        validation_breakdown: &mut HashMap<String, std::time::Duration>,
        start_time: Instant,
    ) -> Result<ParsedCertificate, super::super::responses::BoxedCertificateValidationResponse>
    {
        ...
        match parse_certificate_from_pem(cert_content) {
            ...
            Err(e) => {
                validation_breakdown.insert("parsing".to_string(), parse_start.elapsed());
                Err(Box::new(Self::create_error_response(
                    start_time,
                    validation_breakdown.clone(),
    ...
    async fn perform_revocation_checks(
        &self,
        cert_content: &str,
        ...
        let ocsp_result = tls_manager.validate_certificate_ocsp(cert_content, None);
    ...
    fn create_basic_validation_response(
        &self,
        parsed_cert: &ParsedCertificate,
    ...
    fn create_complete_validation_response(
        parsed_cert: &ParsedCertificate,
        ...
        let is_valid = matches!(checks.time, CheckResult::Passed)
            && matches!(checks.ocsp, CheckResult::Passed)
            && matches!(checks.crl, CheckResult::Passed)
            && checks
                .domain
                .as_ref()
                .is_none_or(|c| matches!(c, CheckResult::Passed))
        ...
        CertificateValidationResponse {
            ...
            validation_summary: ValidationSummary {
                parsing: CheckResult::Passed,
                time_validity: checks.time,
                domain_match: checks.domain,
                ca_validation: checks.ca,
                ocsp_status: Some(checks.ocsp),
                crl_status: Some(checks.crl),
            },
            issues,
            performance: ValidationPerformance {
                total_duration: start_time.elapsed(),
    ...
    fn create_certificate_info(parsed_cert: &ParsedCertificate) -> CertificateInfo {
        CertificateInfo {
