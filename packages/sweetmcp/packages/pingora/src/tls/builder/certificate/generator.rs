...
use super::super::responses::{
    CertificateGenerationResponse, CertificateInfo, FileType, GeneratedFile, GenerationIssue,
    IssueSeverity,
};
...
impl CertificateGeneratorWithDomain {
    ...
    pub async fn generate(self) -> CertificateGenerationResponse {
        let params = match self.setup_certificate_params() {
            ...
            Err(msg) => {
                return Self::error_response(
                    msg,
                    Some("Check certificate parameters and domain names".to_string()),
                );
            }
        };

        let key_pair = match CertificateGeneratorWithDomain::generate_key_pair() {
            Ok(kp) => kp,
            Err(msg) => {
                return Self::error_response(
                    msg,
                    Some("Check system entropy and crypto libraries".to_string()),
                );
            }
        };
        ...
        let cert = match self.generate_certificate(&params, &key_pair) {
            ...
            Err(msg) => {
                return Self::error_response(
                    msg,
                    Some("Check certificate parameters and CA authority".to_string()),
                );
            }
        };
        ...
        let certificate_info = self.create_certificate_info();
    ...
    fn save_certificate_files(
        ...
        cert_pem: &str,
        key_pem: &str,
    ) -> Result<Vec<GeneratedFile>, super::super::responses::BoxedCertificateGenerationResponse>
    {
        let mut files_created = vec![];

        if let Some(save_path) = &self.save_path {
            Self::create_save_directory(save_path, cert_pem, key_pem)?;

            let cert_file = save_path.join("cert.pem");
            let key_file = save_path.join("key.pem");

            Self::write_certificate_file(&cert_file, cert_pem, key_pem)?;
            ...
            CertificateGeneratorWithDomain::write_key_file(&key_file, cert_pem, key_pem)?;
            files_created.push(GeneratedFile {
                path: key_file,
    ...
    fn create_save_directory(
        save_path: &std::path::Path,
        cert_pem: &str,
        key_pem: &str,
    ) -> Result<(), super::super::responses::BoxedCertificateGenerationResponse> {
        if let Err(e) = std::fs::create_dir_all(save_path) {
            return Err(Box::new(CertificateGenerationResponse {
                success: false,
    ...
    fn create_certificate_info(&self) -> CertificateInfo {
    ...
    fn setup_certificate_params(&self) -> Result<rcgen::CertificateParams, String> {
        ...
        params.subject_alt_names =
            Self::setup_san_entries(&self.domains).map_err(|e| format!("SAN setup failed: {e}"))?;

        Ok(params)
    }

    /// Generate cryptographic key pair
    fn generate_key_pair() -> Result<rcgen::KeyPair, String> {
