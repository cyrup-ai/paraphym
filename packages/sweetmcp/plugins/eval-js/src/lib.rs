...
impl StoredVirtualMachine {
    fn new() -> Self {
        debug!("Initializing Node.js process manager");
        let mut scope = None;
        ...
        debug!("Node.js process manager initialized successfully");
...
fn eval_python(input: CallToolRequest) -> Result<CallToolResult, Error> {
    ...
    if let Some(Value::String(code)) = args.get("code") {
        debug!("Executing JavaScript script: {} bytes", code.len());
        ...
        match result {
            Ok(output) => {
                debug!("JavaScript execution successful");
                trace!("JavaScript output: {}", output);
                Ok(CallToolResult {
                    is_error: None,
                    content: vec![Content {
                        annotations: None,
            ...
            Err(exc) => {
                ...
                error!("JavaScript execution failed: {}", error_msg);
                Ok(CallToolResult {
                    is_error: Some(true),
                    content: vec![Content {
                        annotations: None,
